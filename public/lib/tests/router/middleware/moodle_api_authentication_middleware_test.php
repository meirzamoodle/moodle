...
final class moodle_api_authentication_middleware_test extends route_testcase {
    ...
    /**
     * A request bearing a valid API key token, with a scope satisfying the route, authenticates the request.
     */
    public function test_api_key_auth_with_valid_token_and_scope_authenticates(): void {
        $this->resetAfterTest();
        ...
        $this->assertEquals(['core_user:user:read'], $handler->capturedrequest->getAttribute('oauth_scopes'));
    }
    ...
    /**
     * A valid OAuth2 bearer token, carrying a scope satisfying the route, logs the user in.
     */
    public function test_oauth2_login_with_valid_scope_authenticates(): void {
        global $USER;
        ...
        $server->method('validateAuthenticatedRequest')
            ->willReturnCallback(fn (ServerRequestInterface $request) => $request
                ->withAttribute('oauth_user_id', (string) $user->id)
                ->withAttribute('oauth_scopes', ['core_user:user:read']));
