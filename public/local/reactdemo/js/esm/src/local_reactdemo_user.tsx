...
import {fetchRoute, getStrings} from '@moodle/lms/core/amd';
...
/**
 * Fetches and displays a greeting for the given user via the Moodle Router API.
 *
...
export default function User({userid}: Props) {
    ...
    useEffect(() => {
        fetchRoute<{name: string}>('local_reactdemo', `greeting/${userid}`)
            .then((response) => setName(response.name))
            .catch((e) => {
                window.console.error('Failed to load user', e);
                setError(true);
            });
    }, [userid]);
