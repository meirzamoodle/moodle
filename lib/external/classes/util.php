...
class util {
    ...
    public static function generate_token(
        ...
        if (!$name) {
            ...
            $name = self::generate_token_name($newtoken->token);
        }
        $newtoken->name = $name;
    ...
    public static function generate_token_for_current_user(stdClass $service) {
        ...
        if (count($tokens) > 0) {
            ...
            if (
                ...
                $token->name = self::generate_token_name($token->token);
                // Generate the private token, it must be transmitted only via https.
     ...
     * Generate token name.
     *
     * @param string $token Tokens string.
     * @return string
     */
    public static function generate_token_name(string $token): string {
        return get_string(
            'tokennameprefix',
            'webservice',
            random_string(5)
