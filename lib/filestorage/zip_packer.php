...
class zip_packer extends file_packer {
    ...
    private function get_area_maxbytes(int $contextid, string $component): int {
        ...
        if ($context->contextlevel === CONTEXT_USER) {
            ...
            $fileareainfo = file_get_file_area_info(
                contextid: $contextid,
                component: $component,
                filearea: 'private',
            );
        ...
        if ($course) {
            ...
            return $maxbytes !== 0 ? $course->maxbytes : get_user_max_upload_file_size($context);
        }

        // If no conditions match, forbid the unzip process by returning the maximum bytes with 0.
        return 0;
    }
