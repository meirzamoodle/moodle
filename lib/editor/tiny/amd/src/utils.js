 ...
 *
 * @param {TinyMCE} editor
 * @param {object} submenuitem The text of sub-menu that we want to removed
 */
export const removeSubmenuItem = async(editor, submenuitem) => {
    submenuitem.forEach(function (value) {
        let [section, submenuitemtitle] = value;
        // Get menu items.
        ...
        if (menuItems) {
            editor.ui.registry.addNestedMenuItem(
                section,
                {
                    ...
                    getSubmenuItems: () => {
                        let newSubmenu;
                        if (typeof menuItems.getSubmenuItems()  === "string") {
                            // string, the submenu is in disable/inactive status on load.
                            newSubmenu = menuItems.getSubmenuItems().replace(submenuitemtitle, '');
                        } else if (typeof menuItems.getSubmenuItems()  === "object") {
                             // object, the submenu is in enable/active status on load.
                            newSubmenu = [];
