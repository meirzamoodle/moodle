...
/**
 * Turn overflow-y of the course index drawers menu to "unset" value.
 *
 * Unsetting overflow-y if the fullscreen view is activated to make the Tiny menu correctly displayed.
 *
 * @param {bool} state
 */
const unsetDrawers = (state) => {
    const pagedrawers = document.querySelector(' #page.drawers');
    if (pagedrawers) {
        pagedrawers.style.overflowY = state ? "unset" : "auto";
    }
};
