...
class ratelimiter {
    ...
    private function check_limit(string $key, int $ratelimit, int $currenttime): bool {
        $rate_data = $this->cache->get($key);

        if ($rate_data === false) {
            // No data found, initialize rate data.
            $rate_data = ['count' => 0, 'start_time' => $currenttime];
        }

        // Remove expired rate data.
        if ($currenttime - $rate_data['start_time'] > self::TIME_WINDOW) {
            $rate_data['count'] = 0;
            $rate_data['start_time'] = $currenttime;
        }

        // Check rate limit.
        if ($rate_data['count'] < $ratelimit) {
            $rate_data['count']++;
            $this->cache->set($key, $rate_data);
            return true;
        }
